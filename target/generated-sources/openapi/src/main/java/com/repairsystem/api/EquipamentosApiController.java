...
public class EquipamentosApiController implements EquipamentosApi {

    private final EquipamentosApiDelegate delegate;

    public EquipamentosApiController(@Autowired(required = false) EquipamentosApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new EquipamentosApiDelegate() {});
    }

    @Override
    public EquipamentosApiDelegate getDelegate() {
        return delegate;
    }
